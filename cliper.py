...
# Salvar o clipe em um arquivo .avi
def salvar_clip(frames, fps, resolucao):
...
with mss.mss() as sct:
    ...
    monitor = sct.monitors[2]  # escolhe qual monitor capturar
    ...
    while True:
        ...
        # Captura a tela e retorna uma imagem
        img_mss = sct.grab(monitor)

        # Converte a imagem capturada para um array
        frame = np.array(img_mss)

        # Converte o formato de cor de BGRA para BGR
        # frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)

        # Adiciona o frame à nossa fila
